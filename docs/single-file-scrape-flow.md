...
2. 普通多文件仍按现有目录层级模板计算目标文件夹，但基准是设置中的扫描目录，而不是输出目录。目标与旧文件夹不一致时，将旧文件夹内所有条目逐个移动到扫描目录下的新文件夹，保留原文件名并删除空的旧文件夹；目标与旧文件夹一致时不移动。多分片库则按 `movie.nfo` 识别，重命名文件夹与全部分片资产，视频基础名与最终文件夹名保持一致。基准目录的判定统一由 `resolveUpdateModeScanBase` 完成：源文件夹位于配置的扫描目录内（含源文件夹就是扫描目录本身，即视频直接放在扫描根下的情况）时取扫描目录；否则退回源文件夹的父目录。视频直接位于扫描目录根部时，基准必须是扫描目录本身而不是其父目录（盘符根），否则目标会被解析到媒体库外。
3. 封面与预览图分开判断：旧文件夹已存在 `poster.jpg` 时复用该海报，
   不重新下载或提取封面，也不覆盖用户主动编辑后的结果；`fanart.jpg` 缺失时不触发封面下载。
4. NFO 不重新生成。`NfoUpdater` 只修改旧文件中已存在的受管字段，未知字段与旧文件中缺失的字段保持不动；新旧值一致时不写入文件。多分片场景只重命名既有分片 NFO，不覆盖其内容；缺失时才按主 NFO 生成。
...
方括号包围番号后接标题的文件同样会归组，并从文件名末尾识别尾部标签。例如 `[FC2-3259498] 「タイトル」-cd1.mp4` 归组到 `FC2-3259498` 并识别为 `cd1` 分段；写盘时会按“多分段后缀”设置统一输出。`-C`、`-c`、`-U`、`-u` 会写入 `Video.version`，并继续沿用原多版本命名行为。

写盘阶段把同一番号的多文件分成两类处理。

**多分片**：尾部标签支持两类：`cd`、`dvd`、`part`、`pt`、`disc`、`disk` 加数字；或分隔符后的单个纯数字（`1` 至 `9`）。纯数字限制为一位是为了避免把 `011225_01` 这类完整数字番号误判成 `011225` 的分段，后者会按数字番号整体识别。Jellyfin 官方还允许 `a-d` 字母编号，但本应用有意不启用，因为 `C/c` 已用于字幕版本。

写盘输出统一使用设置页“目录”标签中的“多分段后缀”，可选值为 `cd`、`dvd`、`part`、`pt`、`disc`、`disk`，默认 `cd`。以下示例输入为 `cd1` 至 `cd4`，设置为 `disc`，目录层级模板为 `[{num}] {title}`：

```text
[FC2-4694056] 马尾美女/
  movie.nfo
  [FC2-4694056] 马尾美女 - disc1.mp4
  [FC2-4694056] 马尾美女 - disc2.mp4
  [FC2-4694056] 马尾美女 - disc2.nfo
  [FC2-4694056] 马尾美女 - disc2-poster.jpg
  [FC2-4694056] 马尾美女 - disc2-fanart.jpg
  poster.jpg
  fanart.jpg
  extrafanart/
```
...
1. 创建影片目录时完整使用设置中的 `folderLayers`；配置 `[{num}] {title}` 时，最终文件夹名就是 `[FC2-4694056] 马尾美女`。开启 `createMovieFolders` 且模板非空时，视频基础名与最终文件夹名一致；模板为空或未创建影片目录时，退回番号基础名。
2. `movie.nfo` 是整套影片的主 NFO，也服务第一段。刮削结果先按原规则写入该文件。
3. 第二段起生成与视频同名的分片 NFO，例如 `[FC2-4694056] 马尾美女 - disc2.nfo`。内容复制主 NFO，只把 `/movie/title` 改为 `disc2`；后续分段依此类推。第一段不生成专属 NFO。
4. `poster.jpg` 与 `fanart.jpg` 是整套影片图片。第一段不生成专属图片；第二段起先生成 `<视频完整基础名>-poster.<扩展名>` 与 `<视频完整基础名>-fanart.<扩展名>`。若存在 `extrafanart/fanart2.jpg`，则用它同时替换第二段的 poster 和 fanart；fanart3 对应第三段，依此类推。
5. 修改主 NFO 标题或调整多分段后缀后，文件夹、全部分片视频、分片 NFO 与分片图片同步重命名，只改文件名不改内容。更新模式同样遵守该联动规则，且不覆盖已存在的用户图片。
6. 扫描已整理库时，NFO 匹配优先级为 `movie.nfo`、视频同名 NFO、文件夹同名 NFO，保证所有分段都能挂到同一套主元数据。

**多版本**：`1080p`、`4K`、`Director's Cut` 等标签仍按旧规则处理，目录使用父层模板加纯番号，视频使用番号加版本标签；字幕/-无码后缀保留在输出文件名末尾并写入 `Video.version`。与分片混排时，无分片标签的字幕文件也会获得不冲突的配置后分段编号，输出为 `disc2-C` 这类名称。

单文件刮削与批量刮削共用同一套写盘计划，因此从任务列表执行“单独刮削”时也会一次处理该番号下的全部文件。图库详情页的“刷新/刮削”入口同样复用这套分组逻辑：打开单刮削前会按“源文件同目录 + 同番号”把该目录下的全部视频文件（含已刮削的分片）组合成同一分组。
