...
2. 仍按现有目录层级模板计算目标文件夹，但基准是设置中的扫描目录，而不是输出目录。目标与旧文件夹不一致时，将旧文件夹内所有条目逐个移动到扫描目录下的新文件夹，保留原文件名并删除空的旧文件夹；目标与旧文件夹一致时不移动。基准目录的判定统一由 `resolveUpdateModeScanBase` 完成：源文件夹位于配置的扫描目录内（含源文件夹就是扫描目录本身，即视频直接放在扫描根下的情况）时取扫描目录；否则退回源文件夹的父目录。视频直接位于扫描目录根部时，基准必须是扫描目录本身而不是其父目录（盘符根），否则目标会被解析到媒体库外。
...
方括号包围番号后接标题的文件同样会归组，并从文件名末尾识别尾部标签。例如 `[FC2-3259498] 「タイトル」-cd1.mp4` 归组到 `FC2-3259498` 且保留 `cd1` 分片标签。与字幕版本混用时，`FC2-3259498-C.mp4` 会按无后缀分片输出为 `FC2-3259498 - part1-C.mp4`，`-cd1` 文件输出为 `FC2-3259498 - cd1.mp4`，`-C` 同时写入 `Video.version`。
...
图库详情页的“刷新/刮削”入口同样复用这套分组逻辑：打开单刮削前会按“源文件同目录 + 同番号”把该目录下的全部视频文件（含已刮削的分片）组合成同一分组，因此 `[FC2-4620098]` 目录下的 `FC2-4620098-1.mp4` 至 `FC2-4620098-5.mp4` 会统一搜索并输出为 `FC2-4620098 - part1.mp4` 至 `FC2-4620098 - part5.mp4`，而不是只处理当前单个文件。
